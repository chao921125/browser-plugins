...
### 使用方法
1、打开chrome浏览器，找到扩展或者直接输入chrome://extensions/
...
Chrome扩展API地址：https://developer.chrome.com/extensions
可以修改的有：1、右上侧扩展（插件），2、右键菜单，3、开发工具，4、自定义通知消息，5、重写已有API扩展，6、消息通信，7、存储，8、服务器交互，9、国际化
...
位置pageAction（可以不设置这个属性，针对特殊的网站生效）
	一个置灰的browserAction
2、	右键菜单（contextMenus）
Chrome.contextMenus
...
"css3d"的详细信息请参考CSS 3D Transforms 规范，"webgl"请参考WebGL API。 Chrome 3D功能的支持情况请参考WebGL and 3D graphics。 未来可能会增加更多技术功能的检测指定。
version
...
版本1到2之间的变化细节可以参考manifest_version文档。
